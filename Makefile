...
bootstrap-tools: $(TEMPDIR)
	...
	# we purposefully use the latest version of griffon released
	curl -sSfL https://raw.githubusercontent.com/nextlinux/griffon/main/install.sh | sh -s -- -b $(TEMPDIR)/ v0.6.0
...
unit: $(RESULTSDIR) fixtures ## Run unit tests (with coverage)
	...
	go test  -coverprofile $(COVER_REPORT) $(shell go list ./... | grep -v nextlinux/griffon/test)
...
release: clean-dist CHANGELOG.md ## Build and publish final binaries and packages.
	...
	# TODO: in the future add griffon to generate changelogs
